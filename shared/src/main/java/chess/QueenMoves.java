...
public class QueenMoves implements PieceMoveCalculator{
  ...
  private ChessPiece.PieceType queen;
  ...
  ArrayList<ChessMove> GetAllMoves(ChessBoard board, ChessPosition position){
    queen = board.getPiece(position).getPieceType();
...
//      }
  ...
  ChessPosition RepeatedCode(ChessBoard board, ChessPosition startPosition, int row, int col, String Direction){
    ...
    if(board.getPiece(newMovePosition) != null){
      ...
      switch (Direction){
        case "UL":
          return UL(board, newMovePosition);
        case "UR":
          return UR(board, newMovePosition);
        case "DL":
          return DL(board, newMovePosition);
        case "DR":
          return DR(board, newMovePosition);
        case "Up":
